	...
	.equ STATE_RENDER_ORIGIN_X, 55
	.equ STATE_RENDER_ORIGIN_Y, 58
	.equ STATE_SIZE, 61
	.equ GRID_FAR_X, 0
	.equ GRID_NEAR_X, 2
	...
	.global _render_asm_cast_wall_render_begin
	.global _render_asm_cast_wall_render_continue
	.type _render_asm_cast_wall_begin, @function
	.type _render_asm_cast_wall_continue, @function
	.type _render_asm_cast_wall_render_begin, @function
	.type _render_asm_cast_wall_render_continue, @function
...
/*
 * Renderer-specialized ABI.  The camera origin and hit target are invariant
 * for all 80 screen rays and are published once by game_render.  render_column
 * writes only ray_x/ray_y before the first segment; portal transforms mutate
 * those same persistent fields before continuation.  These entries therefore
 * need neither a C argument block nor an IX frame.
 */
_render_asm_cast_wall_render_begin:
	ld hl, (_render_ray_state + STATE_RENDER_ORIGIN_X)
	ld (_render_ray_state + STATE_ORIGIN_X), hl
	ld hl, (_render_ray_state + STATE_RENDER_ORIGIN_Y)
	ld (_render_ray_state + STATE_ORIGIN_Y), hl
	...
	ld a, (_render_ray_state + STATE_RAY_X + 2)
	bit 7, a
	jr z, .Lrender_begin_step_x_ready
	ld hl, -1
.Lrender_begin_step_x_ready:
	ld (_render_ray_state + STATE_MAP_STEP_X), hl
	ld hl, 16
	ld a, (_render_ray_state + STATE_RAY_Y + 2)
	bit 7, a
	jr z, .Lrender_begin_step_y_ready
	ld hl, -16
.Lrender_begin_step_y_ready:
	ld (_render_ray_state + STATE_MAP_STEP_Y), hl

	ld iy, (_render_ray_state + STATE_HIT)
	jr .Lcast_seed_segment

_render_asm_cast_wall_render_continue:
	ld iy, (_render_ray_state + STATE_HIT)
...
.Lstate_component_one:
	...
	.size _render_asm_cast_wall_render_begin, .-_render_asm_cast_wall_render_begin
	.size _render_asm_cast_wall_render_continue, .-_render_asm_cast_wall_render_continue

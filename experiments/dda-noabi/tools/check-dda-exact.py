...
EXPECTED_STATE_LAYOUT = {
    ...
    "STATE_RENDER_ORIGIN_X": 55,
    "STATE_RENDER_ORIGIN_Y": 58,
    "STATE_SIZE": 61,
}
...
def parse_state_layout(assembly_path: Path) -> None:
    ...
    render_begin = re.search(
        r"^_render_asm_cast_wall_render_begin:\s*$"
        r"(?P<body>.*?)"
        r"^_render_asm_cast_wall_render_continue:\s*$",
        source,
        flags=re.MULTILINE | re.DOTALL,
    )
    render_continue = re.search(
        r"^_render_asm_cast_wall_render_continue:\s*$"
        r"(?P<body>.*?)"
        r"^\.Lcast_seed_segment:\s*$",
        source,
        flags=re.MULTILINE | re.DOTALL,
    )
    if render_begin is None or render_continue is None:
        raise AssertionError("renderer-specialized DDA entry points not found")
    for label, body in (
        ("render begin", render_begin.group("body")),
        ("render continue", render_continue.group("body")),
    ):
        if "__frameset" in body or re.search(r"\(ix\s*\+", body):
            raise AssertionError(f"{label} unexpectedly uses the C stack frame")
    begin_body = render_begin.group("body")
    for required in (
        "STATE_RENDER_ORIGIN_X",
        "STATE_RENDER_ORIGIN_Y",
        "STATE_RAY_X",
        "STATE_RAY_Y",
        "STATE_HIT",
    ):
        if required not in begin_body:
            raise AssertionError(f"render begin does not consume {required}")

	...
	.equ DEFERRED_DESTINATION, 0
	.equ DEFERRED_TEXTURE, 3
	.equ DEFERRED_BOUNDARIES, 6
	.equ DEFERRED_COLORS, 9
	.equ DEFERRED_START, 12
	.equ DEFERRED_END, 13
	.equ DEFERRED_FLAGS, 14
	.equ DEFERRED_SIZE, 15
...
.Lportal_transform_flags:
	...
	.global _render_asm_draw_deferred_walls
	.type _render_asm_draw_wall_segment, @function
	.type _render_asm_draw_deferred_walls, @function
 ...
 *   ix+18 WallContext *
 */
_render_asm_draw_deferred_walls:
	call __frameset0
	ld hl, (ix + 6)
	ld (_render_deferred_jobs), hl
	ld a, (ix + 9)
	or a, a
	jr z, .Ldeferred_done
	ld (_render_deferred_count), a
	ld a, 1
	ld (_render_span_batch), a

.Ldeferred_load:
	ld iy, (_render_deferred_jobs)
	ld hl, (iy + DEFERRED_DESTINATION)
	ld (_render_deferred_destination), hl
	ld hl, (iy + DEFERRED_TEXTURE)
	ld (_render_span_texture), hl
	ld hl, (iy + DEFERRED_BOUNDARIES)
	ld (_render_span_boundaries), hl
	ld hl, (iy + DEFERRED_COLORS)
	ld (_render_span_colors), hl
	ld a, (iy + DEFERRED_START)
	ld (_render_span_y), a
	ld a, (iy + DEFERRED_END)
	ld (_render_span_end), a
	ld a, (iy + DEFERRED_FLAGS)
	ld (_render_span_material), a
	ld iy, (_render_deferred_destination)

	/* Locate the first source row exactly as the direct column entry does. */
	ld a, (_render_span_y)
	ld c, a
	ld b, 0
	ld hl, (_render_span_boundaries)
	cp a, (hl)
	jr z, .Ldeferred_initial_ready
.Ldeferred_initial_seek:
	ld a, b
	cp a, 7
	jr nc, .Ldeferred_initial_ready
	inc hl
	ld a, c
	cp a, (hl)
	jr c, .Ldeferred_initial_ready
	inc b
	jr .Ldeferred_initial_seek
.Ldeferred_initial_ready:
	ld a, b
...
.Lspan_return:
	ld a, (_render_span_batch)
	or a, a
	jr nz, .Ldeferred_next
	pop ix
	ret

.Ldeferred_next:
	ld hl, (_render_deferred_jobs)
	ld de, DEFERRED_SIZE
	add hl, de
	ld (_render_deferred_jobs), hl
	ld a, (_render_deferred_count)
	dec a
	ld (_render_deferred_count), a
	jp nz, .Ldeferred_load
	jp .Ldeferred_done

	.size _render_asm_draw_wall_segment, .-_render_asm_draw_wall_segment
	.size _render_asm_draw_deferred_walls, .-_render_asm_draw_deferred_walls
...
_render_span_texel:
	.zero 1
	.local _render_span_batch
_render_span_batch:
	.zero 1
	.local _render_deferred_jobs
_render_deferred_jobs:
	.zero 3
	.local _render_deferred_count
_render_deferred_count:
	.zero 1
	.local _render_deferred_destination
_render_deferred_destination:
	.zero 3

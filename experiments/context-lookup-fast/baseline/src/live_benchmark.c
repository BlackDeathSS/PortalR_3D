#include "live_benchmark.h"
#include "game.h"

#include <fileioc.h>
#include <graphx.h>
#include <stdint.h>
#include <string.h>
#include <sys/timers.h>
#include <ti/getcsc.h>
#include <ti/screen.h>
#include <time.h>

#ifndef RENDER_LIVE_BENCHMARK
#define RENDER_LIVE_BENCHMARK 0
#endif
#ifndef LIVE_BENCHMARK_AUTOTEST_HOLD
#define LIVE_BENCHMARK_AUTOTEST_HOLD 0
#endif

#if RENDER_LIVE_BENCHMARK

#define LIVE_MAGIC "P3DLIV1"
#define LIVE_FORMAT_VERSION 1u
#define LIVE_RESULT_NAME "P3DLIVE"
#define LIVE_TEMP_NAME "P3DLTMP"

#define LIVE_HEADER_SIZE 96u
#define LIVE_SECTION_RECORD_SIZE 84u
#define LIVE_FRAME_RECORD_SIZE 24u
#define LIVE_SECTION_COUNT 18u
#define LIVE_FRAME_COUNT 971u
#define LIVE_WARMUP_FRAMES 16u
/* MOVE_SPEED=600 crosses three linked portals on the versioned live route. */
#define LIVE_EXPECTED_CROSSINGS 3u
#define LIVE_REPORT_SIZE \
    (LIVE_HEADER_SIZE + \
        LIVE_SECTION_COUNT * LIVE_SECTION_RECORD_SIZE + \
        LIVE_FRAME_COUNT * LIVE_FRAME_RECORD_SIZE)

#define LIVE_TICKS_PER_SECOND 30u
#define LIVE_ELAPSED_TICKS 1u
#define LIVE_TRACE_TIMER_HZ CLOCKS_PER_SEC

#define LIVE_FLAG_FRAME_TIMINGS      (1UL << 0)
#define LIVE_FLAG_FIXED_TIMESTEP     (1UL << 1)
#define LIVE_FLAG_FULL_GAME_UPDATE   (1UL << 2)
#define LIVE_FLAG_FULL_RENDER_SWAP   (1UL << 3)
#define LIVE_FLAG_SECTION_TRACE      (1UL << 4)
#define LIVE_FLAG_FRAME_HASHES       (1UL << 5)
#define LIVE_FLAG_STARTUP_TIMING     (1UL << 6)
#define LIVE_FLAG_DIAGNOSTICS_EXCLUDED (1UL << 7)
#define LIVE_FLAG_ARCHIVE_RESULT     (1UL << 8)
#define LIVE_FLAG_DIAGNOSTIC_REPLAY  (1UL << 9)

#define LIVE_SECTION_SMALL_ROOM      (1u << 0)
#define LIVE_SECTION_LARGE_ROOM      (1u << 1)
#define LIVE_SECTION_SLOW_TURN       (1u << 2)
#define LIVE_SECTION_FAST_TURN       (1u << 3)
#define LIVE_SECTION_PORTAL_VIEW     (1u << 4)
#define LIVE_SECTION_PORTAL_APPROACH (1u << 5)
#define LIVE_SECTION_PORTAL_CROSS    (1u << 6)
#define LIVE_SECTION_STRESS          (1u << 7)

#define LIVE_STEP_PULSE_TURN (1u << 0)

#define LIVE_FRAME_CROSSED_PORTAL (1u << 0)
#define LIVE_FRAME_DETAILED       (1u << 1)
#define LIVE_FRAME_CHANGED        (1u << 2)
#define LIVE_FRAME_SECTION_END    (1u << 3)

#define LIVE_CROSS_DISTANCE 64

typedef struct {
    const char *name;
    uint8_t flags;
} LiveSection;

typedef struct {
    uint16_t frames;
    int8_t move_axis;
    int8_t turn_axis;
    uint8_t section;
    uint8_t flags;
} LiveRouteStep;

typedef struct {
    uint8_t step;
    uint16_t step_frame;
    uint16_t route_frame;
} LiveController;

static const LiveSection live_sections[LIVE_SECTION_COUNT] = {
    {"A_APPROACH", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS},
    {"LARGE_ENTRY", LIVE_SECTION_LARGE_ROOM},
    {"SLOW_SWEEP", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_SLOW_TURN |
        LIVE_SECTION_PORTAL_VIEW},
    {"FAST_SWEEP", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_FAST_TURN |
        LIVE_SECTION_PORTAL_VIEW | LIVE_SECTION_STRESS},
    {"A_RETURN", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS},
    {"B_APPROACH", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_APPROACH},
    {"B_VIEW", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_FAST_TURN},
    {"B_TO_SMALL", LIVE_SECTION_SMALL_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS},
    {"SMALL_SWEEP", LIVE_SECTION_SMALL_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_FAST_TURN | LIVE_SECTION_STRESS},
    {"B_RETURN", LIVE_SECTION_SMALL_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS},
    {"OPEN_TRAVERSE", LIVE_SECTION_LARGE_ROOM},
    {"OPEN_NAV", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_FAST_TURN},
    {"D_APPROACH", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH},
    {"D_CROSS", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS |
        LIVE_SECTION_STRESS},
    {"D_TURN_BACK", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_FAST_TURN | LIVE_SECTION_STRESS},
    {"D_RETURN", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_PORTAL_APPROACH | LIVE_SECTION_PORTAL_CROSS |
        LIVE_SECTION_STRESS},
    {"BOUNDARY_SLOW", LIVE_SECTION_LARGE_ROOM | LIVE_SECTION_PORTAL_VIEW |
        LIVE_SECTION_SLOW_TURN | LIVE_SECTION_STRESS},
    {"OPEN_EXIT", LIVE_SECTION_LARGE_ROOM}
};

/*
 * This is an RLE input recording, not a camera keyframe list.  Every route
 * frame is passed through game_update(), game_render(), and gfx_SwapDraw().
 * A pulsed turn applies input on alternating frames for a slow camera sweep.
 */
static const LiveRouteStep live_route[] = {
    {8, 1, 0, 0, 0},
    {20, 1, 0, 1, 0},
    {80, 0, 1, 2, LIVE_STEP_PULSE_TURN},
    {40, 0, 1, 3, 0},
    {28, 1, 0, 4, 0},
    {40, 0, 1, 5, 0},
    {36, 1, 0, 5, 0},
    {40, 0, 1, 6, 0},
    {20, 1, 0, 7, 0},
    {80, 0, 1, 8, 0},
    {20, 1, 0, 9, 0},
    {120, 1, 0, 10, 0},
    {40, 0, 1, 11, 0},
    {24, 1, 0, 11, 0},
    {40, 0, -1, 11, 0},
    {105, 1, 0, 12, 0},
    {20, 1, 0, 13, 0},
    {80, 0, 1, 14, 0},
    {10, 1, 0, 15, 0},
    {80, 0, 1, 16, LIVE_STEP_PULSE_TURN},
    {40, 1, 0, 17, 0}
};

#define LIVE_ROUTE_STEP_COUNT \
    ((uint8_t)(sizeof(live_route) / sizeof(live_route[0])))

static uint8_t *live_report;
static uint8_t live_report_handle;
static GameState live_game;

_Static_assert(LIVE_REPORT_SIZE < 65536u,
    "Live benchmark report must fit in one AppVar");
_Static_assert(GAME_RENDER_BENCH_CATEGORY_COUNT == 7,
    "Live benchmark format requires seven render categories");

static void write_u16(uint8_t *destination, uint16_t value) {
    destination[0] = (uint8_t)value;
    destination[1] = (uint8_t)(value >> 8);
}

static void write_u24(uint8_t *destination, uint32_t value) {
    destination[0] = (uint8_t)value;
    destination[1] = (uint8_t)(value >> 8);
    destination[2] = (uint8_t)(value >> 16);
}

static void write_s24(uint8_t *destination, fixed_t value) {
    write_u24(destination, (uint32_t)value & 0xFFFFFFUL);
}

static void write_u32(uint8_t *destination, uint32_t value) {
    destination[0] = (uint8_t)value;
    destination[1] = (uint8_t)(value >> 8);
    destination[2] = (uint8_t)(value >> 16);
    destination[3] = (uint8_t)(value >> 24);
}

static void live_put_hex32(uint32_t value) {
    static const char digits[] = "0123456789ABCDEF";
    char text[9];
    uint8_t index = 8;

    text[8] = '\0';
    do {
        text[--index] = digits[value & 0x0Fu];
        value >>= 4;
    } while (index != 0);
    os_PutStrFull(text);
}

static uint16_t clamp_u16(uint32_t value) {
    return value > 65535UL ? 65535u : (uint16_t)value;
}

static uint32_t live_crc32(const uint8_t *data, uint16_t size) {
    uint32_t crc = 0xFFFFFFFFUL;
    uint16_t index;

    for (index = 0; index < size; ++index) {
        uint8_t bit;

        crc ^= data[index];
        for (bit = 0; bit < 8; ++bit) {
            uint32_t mask = (uint32_t)-(int32_t)(crc & 1u);

            crc = (crc >> 1) ^ (0xEDB88320UL & mask);
        }
    }
    return ~crc;
}

static uint32_t live_hash_bytes(
    uint32_t hash,
    const uint8_t *data,
    uint24_t size
) {
    uint24_t index;

    for (index = 0; index < size; ++index) {
        hash ^= data[index];
        hash *= 16777619UL;
    }
    return hash;
}

static uint32_t live_frame_hash(void) {
    return live_hash_bytes(
        2166136261UL,
        &gfx_vbuffer[0][0],
        GFX_LCD_WIDTH * GFX_LCD_HEIGHT
    );
}

static uint32_t live_game_hash(const GameState *game) {
    uint8_t state[17];

    write_s24(state + 0, game->player_x);
    write_s24(state + 3, game->player_y);
    write_u16(state + 6, game->angle);
    memcpy(state + 8, &game->primary, sizeof(Portal));
    memcpy(state + 12, &game->secondary, sizeof(Portal));
    state[16] = game->previous_buttons;
    return live_hash_bytes(2166136261UL, state, sizeof(state));
}

static uint32_t live_build_fingerprint(void) {
    return GAME_BUILD_VERSION;
}

static uint32_t live_route_fingerprint(void) {
    uint32_t hash = live_hash_bytes(
        2166136261UL,
        (const uint8_t *)live_route,
        sizeof(live_route)
    );
    uint8_t section;

    for (section = 0; section < LIVE_SECTION_COUNT; ++section) {
        hash = live_hash_bytes(
            hash,
            (const uint8_t *)live_sections[section].name,
            (uint24_t)strlen(live_sections[section].name)
        );
        hash ^= live_sections[section].flags;
        hash *= 16777619UL;
    }
    return hash;
}

static uint8_t *live_section_record(uint8_t section) {
    return live_report + LIVE_HEADER_SIZE +
        (uint16_t)section * LIVE_SECTION_RECORD_SIZE;
}

static uint8_t *live_frame_record(uint16_t frame) {
    return live_report + LIVE_HEADER_SIZE +
        LIVE_SECTION_COUNT * LIVE_SECTION_RECORD_SIZE +
        (uint24_t)frame * LIVE_FRAME_RECORD_SIZE;
}

static uint8_t live_prepare_report(void) {
#if LIVE_BENCHMARK_AUTOTEST_HOLD
    /*
     * A failed autotest must not leave the decoder free to rediscover a
     * previous successful payload in calculator RAM.
     */
    uint8_t previous_handle = ti_Open(LIVE_RESULT_NAME, "r+");

    if (previous_handle != 0) {
        uint8_t *previous_report = ti_GetDataPtr(previous_handle);

        if (previous_report != NULL && ti_GetSize(previous_handle) != 0) {
            previous_report[0] = 0;
        }
        ti_Close(previous_handle);
    }
#endif
    (void)ti_Delete(LIVE_TEMP_NAME);
    live_report_handle = ti_Open(LIVE_TEMP_NAME, "w");
    if (live_report_handle == 0) {
        return 0;
    }
    if (ti_Resize(LIVE_REPORT_SIZE, live_report_handle) != LIVE_REPORT_SIZE) {
        ti_Close(live_report_handle);
        live_report_handle = 0;
        (void)ti_Delete(LIVE_TEMP_NAME);
        return 0;
    }
    live_report = ti_GetDataPtr(live_report_handle);
    if (live_report == NULL) {
        ti_Close(live_report_handle);
        live_report_handle = 0;
        (void)ti_Delete(LIVE_TEMP_NAME);
        return 0;
    }
    memset(live_report, 0, LIVE_REPORT_SIZE);
    return 1;
}

static uint8_t live_commit_report(void) {
    ti_Close(live_report_handle);
    live_report_handle = 0;
    live_report = NULL;

    (void)ti_Delete(LIVE_RESULT_NAME);
    return ti_Rename(LIVE_TEMP_NAME, LIVE_RESULT_NAME) == 0;
}

#if !LIVE_BENCHMARK_AUTOTEST_HOLD
static uint8_t live_archive_report(void) {
    uint8_t handle = ti_Open(LIVE_RESULT_NAME, "r+");
    uint8_t archived;

    if (handle == 0) {
        return 0;
    }
    archived = ti_SetArchiveStatus(true, handle) != 0;
    ti_Close(handle);
    return archived;
}
#endif

static void live_reset_game(GameState *game) {
    memset(game, 0, sizeof(*game));
    game_init(game);
}

static void live_controller_reset(LiveController *controller) {
    controller->step = 0;
    controller->step_frame = 0;
    controller->route_frame = 0;
}

static uint8_t live_controller_next(
    LiveController *controller,
    int8_t *move_axis,
    int8_t *turn_axis,
    uint8_t *section
) {
    const LiveRouteStep *step;

    if (controller->step >= LIVE_ROUTE_STEP_COUNT) {
        return 0;
    }
    step = &live_route[controller->step];
    *move_axis = step->move_axis;
    *turn_axis = step->turn_axis;
    if ((step->flags & LIVE_STEP_PULSE_TURN) != 0 &&
        (controller->step_frame & 1u) != 0) {
        *turn_axis = 0;
    }
    *section = step->section;

    ++controller->route_frame;
    ++controller->step_frame;
    if (controller->step_frame == step->frames) {
        ++controller->step;
        controller->step_frame = 0;
    }
    return 1;
}

static uint8_t live_route_is_valid(void) {
    uint16_t total_frames = 0;
    uint8_t previous_section = 0;
    uint8_t step;

    for (step = 0; step < LIVE_ROUTE_STEP_COUNT; ++step) {
        const LiveRouteStep *route_step = &live_route[step];

        if (route_step->section >= LIVE_SECTION_COUNT ||
            (step != 0 && route_step->section < previous_section)) {
            return 0;
        }
        previous_section = route_step->section;
        total_frames += route_step->frames;
    }
    return total_frames == LIVE_FRAME_COUNT &&
        previous_section == LIVE_SECTION_COUNT - 1u;
}

static void live_write_sections(void) {
    uint16_t first_frame = 0;
    uint8_t step = 0;
    uint8_t section;

    for (section = 0; section < LIVE_SECTION_COUNT; ++section) {
        uint8_t *record = live_section_record(section);
        uint16_t frame_count = 0;
        uint8_t name_length = (uint8_t)strlen(live_sections[section].name);

        while (step < LIVE_ROUTE_STEP_COUNT &&
            live_route[step].section == section) {
            frame_count += live_route[step].frames;
            ++step;
        }
        if (name_length > 15u) name_length = 15u;
        record[0] = (uint8_t)(section + 1u);
        record[1] = live_sections[section].flags;
        write_u16(record + 2, first_frame);
        write_u16(record + 4, frame_count);
        write_u16(record + 6, 0xFFFFu);
        memcpy(record + 8, live_sections[section].name, name_length);
        first_frame += frame_count;
    }
}

static fixed_t live_abs(fixed_t value) {
    return value < 0 ? -value : value;
}

static uint8_t live_crossed_portal(
    const GameState *before,
    const GameState *after
) {
    return live_abs(after->player_x - before->player_x) > LIVE_CROSS_DISTANCE ||
        live_abs(after->player_y - before->player_y) > LIVE_CROSS_DISTANCE;
}

static uint8_t live_section_ends(uint16_t frame, uint8_t section) {
    const uint8_t *record = live_section_record(section);
    uint16_t first = (uint16_t)record[2] | ((uint16_t)record[3] << 8);
    uint16_t count = (uint16_t)record[4] | ((uint16_t)record[5] << 8);

    return frame + 1u == first + count;
}

static uint8_t live_should_trace(uint16_t frame, uint8_t section) {
    const uint8_t *record = live_section_record(section);
    uint16_t detail_frame =
        (uint16_t)record[6] | ((uint16_t)record[7] << 8);

    return frame == detail_frame;
}

static void live_write_detail(
    uint8_t section,
    uint16_t frame,
    const GameRenderBenchmark *detail
) {
    uint8_t *record = live_section_record(section);
    uint8_t category;

    write_u16(record + 6, frame);
    for (category = 0;
         category < GAME_RENDER_BENCH_CATEGORY_COUNT;
         ++category) {
        write_u24(record + 24 + category * 3, detail->raw_ticks[category]);
        write_u16(record + 46 + category * 2, detail->entries[category]);
    }
    write_u16(record + 64, detail->portal_transforms);
    write_u16(record + 66, detail->wall_calls);
    write_u16(record + 68, detail->mask_calls);
    write_u16(record + 70, detail->portal_candidates);
    write_u16(record + 72, detail->linked_exits);
    write_u16(record + 74, clamp_u16(detail->textured_rows));
    write_u16(record + 76, clamp_u16(detail->dda_steps));
}

static void live_write_frame(
    uint16_t frame,
    uint8_t section,
    int8_t move_axis,
    int8_t turn_axis,
    uint8_t flags,
    uint32_t update_ticks,
    uint32_t render_ticks,
    uint32_t swap_ticks,
    const GameRenderProfile *profile
) {
    uint8_t *record = live_frame_record(frame);
    uint32_t total_ticks = update_ticks + render_ticks + swap_ticks;

    write_u16(record + 0, clamp_u16(total_ticks));
    write_u16(record + 2, clamp_u16(update_ticks));
    write_u16(record + 4, clamp_u16(render_ticks));
    write_u16(record + 6, clamp_u16(swap_ticks));
    write_s24(record + 8, live_game.player_x);
    write_s24(record + 11, live_game.player_y);
    write_u16(record + 14, live_game.angle);
    write_u16(record + 16, profile->cast_count);
    record[18] = profile->max_depth;
    record[19] = (uint8_t)(section + 1u);
    record[20] = flags;
    record[21] = (uint8_t)(
        ((uint8_t)(move_axis + 1) & 3u) |
        (((uint8_t)(turn_axis + 1) & 3u) << 2)
    );
}

static void live_run_warmup(void) {
    LiveController controller;
    uint8_t frame;

    live_reset_game(&live_game);
    live_controller_reset(&controller);
    for (frame = 0; frame < LIVE_WARMUP_FRAMES; ++frame) {
        int8_t move_axis;
        int8_t turn_axis;
        uint8_t section;

        (void)live_controller_next(
            &controller, &move_axis, &turn_axis, &section
        );
        (void)section;
        (void)game_update(
            &live_game,
            move_axis,
            turn_axis,
            0,
            LIVE_ELAPSED_TICKS,
            LIVE_TICKS_PER_SECOND
        );
        game_render_benchmark_reset();
        game_render(&live_game);
        gfx_SwapDraw();
    }
}

static uint32_t live_render_first_frame(void) {
    clock_t started;
    clock_t elapsed;

    live_reset_game(&live_game);
    game_render_benchmark_reset();
    started = clock();
    game_render(&live_game);
    elapsed = clock() - started;
    gfx_SwapDraw();
    return elapsed;
}

static uint8_t live_run_route(
    uint32_t *total_ticks,
    uint32_t *state_hash,
    uint16_t *crossings,
    uint32_t *wall_ticks
) {
    LiveController controller;
    uint32_t slowest_render_ticks[LIVE_SECTION_COUNT] = {0};
    uint32_t route_hash = 2166136261UL;
    uint16_t frame = 0;
    clock_t wall_started;

    *total_ticks = 0;
    *crossings = 0;
    live_reset_game(&live_game);
    live_controller_reset(&controller);
    wall_started = clock();

    while (frame < LIVE_FRAME_COUNT) {
        GameState before = live_game;
        int8_t move_axis;
        int8_t turn_axis;
        uint8_t section;
        uint8_t changed;
        uint8_t crossed;
        uint8_t section_end;
        uint8_t frame_flags = 0;
        clock_t phase_started;
        uint32_t update_ticks;
        uint32_t render_ticks;
        uint32_t swap_ticks;
        GameRenderProfile profile;

        if (!live_controller_next(
                &controller, &move_axis, &turn_axis, &section
            )) {
            return 0;
        }

        phase_started = clock();
        changed = game_update(
            &live_game,
            move_axis,
            turn_axis,
            0,
            LIVE_ELAPSED_TICKS,
            LIVE_TICKS_PER_SECOND
        );
        update_ticks = clock() - phase_started;
        crossed = live_crossed_portal(&before, &live_game);
        section_end = live_section_ends(frame, section);

        if (changed) frame_flags |= LIVE_FRAME_CHANGED;
        if (crossed) {
            uint8_t *section_record = live_section_record(section);
            uint16_t section_crossings =
                (uint16_t)section_record[82] |
                ((uint16_t)section_record[83] << 8);

            frame_flags |= LIVE_FRAME_CROSSED_PORTAL;
            ++*crossings;
            write_u16(section_record + 82, section_crossings + 1u);
        }
        if (section_end) frame_flags |= LIVE_FRAME_SECTION_END;

        game_render_benchmark_reset();
        phase_started = clock();
        game_render(&live_game);
        render_ticks = clock() - phase_started;
        profile = *game_get_render_profile();

        if (render_ticks > slowest_render_ticks[section]) {
            slowest_render_ticks[section] = render_ticks;
            write_u16(live_section_record(section) + 6, frame);
        }

        if (section_end) {
            uint8_t *section_record = live_section_record(section);

            write_u32(section_record + 78, live_game_hash(&live_game));
        }

        phase_started = clock();
        gfx_SwapDraw();
        swap_ticks = clock() - phase_started;

        live_write_frame(
            frame,
            section,
            move_axis,
            turn_axis,
            frame_flags,
            update_ticks,
            render_ticks,
            swap_ticks,
            &profile
        );
        *total_ticks += update_ticks + render_ticks + swap_ticks;
        route_hash = live_hash_bytes(
            route_hash,
            live_frame_record(frame) + 8,
            10
        );
        ++frame;
    }

    *wall_ticks = clock() - wall_started;
    *state_hash = route_hash;
    {
        uint8_t section;

        for (section = 0; section < LIVE_SECTION_COUNT; ++section) {
            uint8_t *section_record = live_section_record(section);
            uint16_t detail_frame =
                (uint16_t)section_record[6] |
                ((uint16_t)section_record[7] << 8);

            live_frame_record(detail_frame)[20] |= LIVE_FRAME_DETAILED;
        }
    }
    return controller.step == LIVE_ROUTE_STEP_COUNT &&
        controller.route_frame == LIVE_FRAME_COUNT;
}

/*
 * Replay only the selected slowest render in each section and every section
 * endpoint after the continuous timed route.  Detailed trace switches and
 * full-screen hashes therefore cannot stall or contaminate live frame times.
 */
static uint8_t live_run_diagnostics(
    uint32_t *final_frame_hash,
    uint16_t *detailed_frames
) {
    LiveController controller;
    uint16_t replay_crossings = 0;
    uint16_t frame = 0;

    *detailed_frames = 0;
    live_reset_game(&live_game);
    live_controller_reset(&controller);

    while (frame < LIVE_FRAME_COUNT) {
        GameState before = live_game;
        int8_t move_axis;
        int8_t turn_axis;
        uint8_t section;
        uint8_t crossed;
        uint8_t detailed;
        uint8_t section_end;
        uint8_t *frame_record;

        if (!live_controller_next(
                &controller, &move_axis, &turn_axis, &section
            )) {
            return 0;
        }
        (void)game_update(
            &live_game,
            move_axis,
            turn_axis,
            0,
            LIVE_ELAPSED_TICKS,
            LIVE_TICKS_PER_SECOND
        );
        crossed = live_crossed_portal(&before, &live_game);
        detailed = live_should_trace(frame, section);
        section_end = live_section_ends(frame, section);
        frame_record = live_frame_record(frame);

        if (crossed) ++replay_crossings;
        if (((frame_record[20] & LIVE_FRAME_CROSSED_PORTAL) != 0) !=
            (crossed != 0)) {
            return 0;
        }

        if (detailed || section_end) {
            game_render_benchmark_reset();
            if (detailed) game_render_benchmark_begin();
            game_render(&live_game);
            if (detailed) {
                game_render_benchmark_end();
                live_write_detail(
                    section,
                    frame,
                    game_render_benchmark_read()
                );
                ++*detailed_frames;
            }
            if (section_end) {
                uint8_t *section_record = live_section_record(section);

                if (live_game_hash(&live_game) !=
                    ((uint32_t)section_record[78] |
                    ((uint32_t)section_record[79] << 8) |
                    ((uint32_t)section_record[80] << 16) |
                    ((uint32_t)section_record[81] << 24))) {
                    return 0;
                }
                write_u32(section_record + 60, live_frame_hash());
            }
        }
        ++frame;
    }

    {
        const uint8_t *last_section =
            live_section_record(LIVE_SECTION_COUNT - 1u);

        *final_frame_hash =
            (uint32_t)last_section[60] |
            ((uint32_t)last_section[61] << 8) |
            ((uint32_t)last_section[62] << 16) |
            ((uint32_t)last_section[63] << 24);
    }
    return controller.step == LIVE_ROUTE_STEP_COUNT &&
        controller.route_frame == LIVE_FRAME_COUNT &&
        replay_crossings == LIVE_EXPECTED_CROSSINGS;
}

static void live_write_header(
    uint32_t build_fingerprint,
    uint32_t switch_cost_q8,
    uint32_t graphics_init_ticks,
    uint32_t first_frame_ticks,
    uint32_t total_ticks,
    uint32_t state_hash,
    uint32_t final_frame_hash,
    uint16_t crossings,
    uint16_t detailed_frames,
    uint32_t wall_ticks
) {
    uint8_t *header = live_report;
    uint32_t flags =
        LIVE_FLAG_FRAME_TIMINGS |
        LIVE_FLAG_FIXED_TIMESTEP |
        LIVE_FLAG_FULL_GAME_UPDATE |
        LIVE_FLAG_FULL_RENDER_SWAP |
        LIVE_FLAG_SECTION_TRACE |
        LIVE_FLAG_FRAME_HASHES |
        LIVE_FLAG_STARTUP_TIMING |
        LIVE_FLAG_DIAGNOSTICS_EXCLUDED |
        LIVE_FLAG_DIAGNOSTIC_REPLAY
#if !LIVE_BENCHMARK_AUTOTEST_HOLD
        | LIVE_FLAG_ARCHIVE_RESULT
#endif
        ;
    uint32_t body_crc;

    memcpy(header, LIVE_MAGIC, 7);
    write_u16(header + 8, LIVE_FORMAT_VERSION);
    write_u16(header + 10, LIVE_HEADER_SIZE);
    write_u32(header + 12, flags);
    write_u32(header + 16, CLOCKS_PER_SEC);
    write_u32(header + 20, LIVE_TRACE_TIMER_HZ);
    write_u32(header + 24, build_fingerprint);
    write_u32(header + 28, live_route_fingerprint());
    write_u16(header + 32, LIVE_REPORT_SIZE);
    write_u16(header + 34, LIVE_SECTION_COUNT);
    write_u16(header + 36, LIVE_WARMUP_FRAMES);
    write_u16(header + 38, LIVE_FRAME_COUNT);
    write_u16(header + 40, LIVE_SECTION_RECORD_SIZE);
    write_u16(header + 42, LIVE_FRAME_RECORD_SIZE);
    write_u16(header + 44, GAME_RENDER_LOGICAL_COLUMNS);
    header[46] = GAME_RENDER_COLUMN_WIDTH;
    header[47] = GAME_RENDER_TEXTURE_HEIGHT;
    header[48] = GAME_RENDER_MAX_PORTAL_DEPTH;
    header[49] = GAME_RENDER_BENCH_CATEGORY_COUNT;
    write_u16(header + 50, LIVE_TICKS_PER_SECOND);
    write_u16(header + 52, LIVE_ELAPSED_TICKS);
    write_u16(header + 54, LIVE_EXPECTED_CROSSINGS);
    write_u32(header + 56, switch_cost_q8);
    write_u32(header + 64, graphics_init_ticks);
    write_u32(header + 68, first_frame_ticks);
    write_u32(header + 72, total_ticks);
    write_u32(header + 76, state_hash);
    write_u32(header + 80, final_frame_hash);
    write_u16(header + 84, crossings);
    write_u16(header + 86, detailed_frames);
    write_u16(header + 88, LIVE_REPORT_SIZE);
    write_u32(header + 92, wall_ticks);

    body_crc = live_crc32(
        live_report + LIVE_HEADER_SIZE,
        LIVE_REPORT_SIZE - LIVE_HEADER_SIZE
    );
    write_u32(header + 60, body_crc);
}

static void live_show_failure(const char *message) {
    os_ClrHome();
    os_SetCursorPos(0, 0);
    os_PutStrFull("Live benchmark failed");
    os_SetCursorPos(2, 0);
    os_PutStrFull(message);
    os_SetCursorPos(8, 0);
    os_PutStrFull("Press any key");
    while (os_GetCSC() != 0) {
    }
    while (os_GetCSC() == 0) {
    }
}

static void live_show_result(
    uint8_t saved,
    uint8_t archived,
    uint16_t crossings,
    uint32_t build_fingerprint
) {
    char crossing_text[6];
    char *digit = crossing_text + sizeof(crossing_text) - 1;

    *digit = '\0';
    do {
        *--digit = (char)('0' + crossings % 10u);
        crossings /= 10u;
    } while (crossings != 0);

    os_ClrHome();
    os_SetCursorPos(0, 0);
    os_PutStrFull(saved ? "Live benchmark done" : "Live save failed");
    if (saved) {
        os_SetCursorPos(2, 0);
        os_PutStrFull("971 rendered frames");
        os_SetCursorPos(3, 0);
        os_PutStrFull("Portal crossings: ");
        os_PutStrFull(digit);
        os_SetCursorPos(5, 0);
        os_PutStrFull("Format: " LIVE_MAGIC);
        os_SetCursorPos(6, 0);
        os_PutStrFull("Build: 0x");
        live_put_hex32(build_fingerprint);
        os_SetCursorPos(7, 0);
        os_PutStrFull("Result: P3DLIVE");
        os_SetCursorPos(8, 0);
        os_PutStrFull(archived ? "Archived safely" : "Saved in RAM");
    }
    os_SetCursorPos(9, 0);
    os_PutStrFull("Press any key");
    while (os_GetCSC() != 0) {
    }
    while (os_GetCSC() == 0) {
    }
}

int live_benchmark_run(void) {
    uint16_t saved_timer_control;
    uint32_t saved_timer_count;
    uint32_t switch_cost_q8;
    uint32_t graphics_init_ticks;
    uint32_t first_frame_ticks;
    uint32_t total_ticks;
    uint32_t state_hash;
    uint32_t final_frame_hash = 0;
    uint32_t wall_ticks;
    uint32_t build_fingerprint;
    uint16_t crossings;
    uint16_t detailed_frames = 0;
    clock_t started;
    uint8_t route_ok;
    uint8_t diagnostics_ok;
    uint8_t saved;
    uint8_t archived = 0;

    if (!live_route_is_valid()) {
        live_show_failure("Route table invalid.");
        return 1;
    }
    if (!live_prepare_report()) {
        live_show_failure("No room for P3DLIVE.");
        return 1;
    }
    live_write_sections();

    saved_timer_control = timer_Control;
    saved_timer_count = timer_GetSafe(
        3,
        (saved_timer_control & TIMER3_UP) != 0 ? TIMER_UP : TIMER_DOWN
    );
    timer_Disable(3);
    timer_Set(3, 0);
    timer_Enable(3, TIMER_32K, TIMER_NOINT, TIMER_UP);

    gfx_Begin();
    gfx_SetDrawBuffer();
    started = clock();
    game_graphics_init();
    graphics_init_ticks = clock() - started;
    switch_cost_q8 = game_render_benchmark_calibrate();

    first_frame_ticks = live_render_first_frame();
    live_run_warmup();
    route_ok = live_run_route(
        &total_ticks,
        &state_hash,
        &crossings,
        &wall_ticks
    );
    diagnostics_ok = route_ok && live_run_diagnostics(
        &final_frame_hash,
        &detailed_frames
    );
    build_fingerprint = live_build_fingerprint();
    live_write_header(
        build_fingerprint,
        switch_cost_q8,
        graphics_init_ticks,
        first_frame_ticks,
        total_ticks,
        state_hash,
        final_frame_hash,
        crossings,
        detailed_frames,
        wall_ticks
    );

    gfx_End();
    saved = diagnostics_ok && crossings == LIVE_EXPECTED_CROSSINGS &&
        detailed_frames == LIVE_SECTION_COUNT && live_commit_report();
    if (!saved && live_report_handle != 0) {
        ti_Close(live_report_handle);
        live_report_handle = 0;
        (void)ti_Delete(LIVE_TEMP_NAME);
    }
    if (saved) {
#if LIVE_BENCHMARK_AUTOTEST_HOLD
        archived = 0;
#else
        archived = live_archive_report();
#endif
    }

    timer_Disable(3);
    timer_Set(3, saved_timer_count);
    timer_Control = saved_timer_control;

#if LIVE_BENCHMARK_AUTOTEST_HOLD
    for (;;) {
    }
#endif

    live_show_result(saved, archived, crossings, build_fingerprint);
    return saved ? 0 : 1;
}

#endif

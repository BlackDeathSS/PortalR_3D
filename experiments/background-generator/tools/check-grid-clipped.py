...
PACKED_RUN_RATIO = read_equ("GRID_PACKED_RUN_RATIO")
PACKED_RUN_MIN_DX = read_equ("GRID_PACKED_RUN_MIN_DX")
...
def run_raster_model(
    ...
    if (
        dy == 0
        or dx < PACKED_RUN_MIN_DX
        or dx < PACKED_RUN_RATIO * dy
    ):
...
def check_unique_rasters(unique_states: set[int], use_run_path: bool) -> int:
    pixel_checks = 0
    for packed in unique_states:
        ...
        for y0, y1, label in (
            ...
            actual = (
                run_raster_model(x0, y0, x1, y1)
                if use_run_path
                else dynamic_raster_model(x0, y0, x1, y1)
            )
            if expected != actual:
                difference = next(
                    (
...
def check_assembly_contract() -> bool:
    source = ASM_PATH.read_text(encoding="utf-8")
    required = (
    ...
    use_run_path = ".Lgrid_dynamic_run_setup:" in source
    if use_run_path:
        for fragment in (
            ...
            if fragment not in source:
                raise AssertionError(
                    f"missing packed-run assembly fragment: {fragment}"
                )
    return use_run_path
...
def fixed_divide_9(dividend: int, divisor: int) -> tuple[int, int]:
    """Translate the nine fixed trial divisors in the packed-run path."""
    quotient = 0
    remainder = dividend
    trial = divisor << 8
    for _ in range(9):
...
def check_arithmetic_kernels() -> tuple[int, int, int]:
    ...
    fixed_divider_cases = 0
    for dividend in range(SCREEN_WIDTH):
        for divisor in range(1, FLOOR_NEAR_Y - FLOOR_FAR_Y + 1):
            expected = divmod(dividend, divisor)
            actual = fixed_divide_9(dividend, divisor)
            if actual != expected:
                raise AssertionError(
                    "nine-bit divider mismatch: "
                    f"{dividend}/{divisor}: assembly={actual}, expected={expected}"
                )
            fixed_divider_cases += 1

    return multiply_cases, divider_cases, fixed_divider_cases
...
def main() -> None:
    ...
    use_run_path = check_assembly_contract()
    run_cases = check_run_recurrence() if use_run_path else 0
    multiply_cases, divider_cases, fixed_divider_cases = check_arithmetic_kernels()
    if arguments.quick:
        endpoint_min = -512
        endpoint_max = 831
    else:
    ...
    pixel_checks = check_unique_rasters(unique_states, use_run_path)
    ...
    print(
        ...
        f"raster={'packed-run' if use_run_path else 'standard'}, "
        f"{run_cases} run-recurrence, {multiply_cases} multiply, "
        f"{divider_cases} clip-divider and {fixed_divider_cases} fixed9 "
        "run-divider kernel cases"
    )

	...
	.equ BACKGROUND_NEAR_DISTANCE, 260
	.equ BACKGROUND_FAR_DISTANCE, 4096
	.equ BACKGROUND_GRID_LINES, 16
	.equ BACKGROUND_SCREEN_CENTER_Y, 120
	.equ BG_PLAYER_X, 0
	.equ BG_PLAYER_Y, 3
	.equ BG_DIR_X, 6
	.equ BG_DIR_Y, 9
	...
	.equ BG_Y_NEAR, 24
	.equ BG_Y_FAR, 27
	.equ BG_X_LINE_NEAR, 30
	.equ BG_X_LINE_FAR, 33
	.equ BG_X_LINE_STEP, 36
	.equ BG_X_INPUT_NEAR, 39
	.equ BG_X_INPUT_FAR, 42
	.equ BG_Y_LINE_NEAR, 45
	.equ BG_Y_LINE_FAR, 48
	.equ BG_Y_LINE_STEP, 51
	.equ BG_Y_INPUT_NEAR, 54
	.equ BG_Y_INPUT_FAR, 57
	.equ BG_LINE_COUNT, 60
	.equ BG_STATE_SIZE, 61
...
.Lstate_component_one:
	...
	.global _render_asm_add_projected_grid_segment_registers
	.type _render_asm_add_projected_grid_segment, @function
	.type _render_asm_add_projected_grid_segment_registers, @function
...
_render_asm_add_projected_grid_segment:
	...
	ld hl, (ix + 6)
	ld de, (ix + 9)
	call _render_asm_add_projected_grid_segment_registers
	pop ix
	ret

/* Internal hot ABI: HL=lateral_near, DE=lateral_far; IX may be clobbered. */
_render_asm_add_projected_grid_segment_registers:
	ld (_render_grid_input_near), hl
	ld (_render_grid_input_far), de
	 ...
	 */
	ld hl, (_render_grid_input_near)
	ld a, (_render_grid_input_near + 2)
	bit 7, a
...
.Lgrid_project_core_clip_negative:
	...
	.size _render_asm_add_projected_grid_segment_registers, .-_render_asm_add_projected_grid_segment_registers

	.section .text._render_asm_draw_background_grid,"ax",@progbits
	.global _render_asm_draw_background_grid
	.type _render_asm_draw_background_grid, @function

/*
 * Generate and draw all 16+16 world-grid lines in one assembly transition.
 *
 ...
 *   ix+12 direction_y
 *
 * Every value needed after setup lives in fixed scratch.  The projected-line
 * core is therefore free to use IX for its packed-run divider; the original
 * caller frame is restored only once when this whole background phase ends.
 */
_render_asm_draw_background_grid:
	call __frameset0
	ld iy, (ix + 6)
	ld hl, (iy + 0)
	ld (_render_background_state + BG_PLAYER_X), hl
	ld hl, (iy + 3)
	ld (_render_background_state + BG_PLAYER_Y), hl
	ld hl, (ix + 9)
	ld (_render_background_state + BG_DIR_X), hl
	ld hl, (ix + 12)
	ld (_render_background_state + BG_DIR_Y), hl

	ld hl, (_render_background_state + BG_DIR_X)
	call .Lbackground_inverse
	ld (_render_background_state + BG_INV_X), hl
	ld hl, (_render_background_state + BG_DIR_Y)
	call .Lbackground_inverse
	ld (_render_background_state + BG_INV_Y), hl

	/* Exact signed trunc(direction*260/256), and exact direction*16. */
	ld hl, (_render_background_state + BG_DIR_X)
	ld de, BACKGROUND_NEAR_DISTANCE
	call .Lbackground_scale_mul
	ld (_render_background_state + BG_X_NEAR), hl
	ld hl, (_render_background_state + BG_DIR_X)
	.rept 4
	add hl, hl
	.endr
	ld (_render_background_state + BG_X_FAR), hl

	ld hl, (_render_background_state + BG_DIR_Y)
	ld de, BACKGROUND_NEAR_DISTANCE
	call .Lbackground_scale_mul
	ld (_render_background_state + BG_Y_NEAR), hl
	ld hl, (_render_background_state + BG_DIR_Y)
	.rept 4
	add hl, hl
	.endr
	ld (_render_background_state + BG_Y_FAR), hl

	call .Lbackground_init_x_lines
	call .Lbackground_init_y_lines
	call _render_asm_clear_background

	/* Exact cardinals become full-width horizontal pairs. */
	ld hl, (_render_background_state + BG_DIR_Y)
	ld de, 0
	or a, a
	sbc hl, de
	jr z, .Lbackground_x_axis
	call .Lbackground_draw_x_projected
	jr .Lbackground_y_dispatch
.Lbackground_x_axis:
	call .Lbackground_draw_x_axis

.Lbackground_y_dispatch:
	ld hl, (_render_background_state + BG_DIR_X)
	ld de, 0
	or a, a
	sbc hl, de
	jr z, .Lbackground_y_axis
	call .Lbackground_draw_y_projected
	jr .Lbackground_done
.Lbackground_y_axis:
	call .Lbackground_draw_y_axis

.Lbackground_done:
	pop ix
	ret

/* Return abs(65536/component), with zero mapped to zero for this caller. */
.Lbackground_inverse:
	ld (_render_background_mul_value), hl
	ld a, (_render_background_mul_value + 2)
	bit 7, a
	jr z, .Lbackground_inverse_magnitude
	call .Lbackground_negate_hl
.Lbackground_inverse_magnitude:
	ld de, 0
	or a, a
	sbc hl, de
	ret z
	dec hl
	jr z, .Lbackground_inverse_one
	inc hl
	add hl, hl
	ld de, _render_reciprocal_delta
	add hl, de
	...
	ld h, d
	ret
.Lbackground_inverse_one:
	ld hl, 65536
	ret

/* HL=-HL, exact for every reachable background value. */
.Lbackground_negate_hl:
	ex de, hl
	or a, a
	sbc hl, hl
	sbc hl, de
...
/*
 * HL=signed value, DE=positive 8.8 factor. Return trunc(HL*DE/256).
 *
 * Reachable magnitudes fit 16 bits. Split magnitude into low/high bytes:
 *   floor(m*f/256) = floor(m.low*f/256) + m.high*f.
 * The first term reuses the exact two-MLT DDA scaler; two more MLTs form the
 * second term.  A zero byte before a stored 16-bit product shifts it left by
 * eight without an eight-instruction shift loop.
 */
.Lbackground_scale_mul:
	ld (_render_background_mul_value), hl
	ld (_render_background_mul_factor), de
	xor a, a
	ld (_render_background_mul_negative), a
	ld a, (_render_background_mul_value + 2)
	bit 7, a
	jr z, .Lbackground_scale_magnitude_ready
	ld a, 1
	ld (_render_background_mul_negative), a
	ld hl, (_render_background_mul_value)
	call .Lbackground_negate_hl
	ld (_render_background_mul_value), hl
.Lbackground_scale_magnitude_ready:
	/* The only reachable 17-bit factor is the exact reciprocal of one. */
	ld a, (_render_background_mul_factor + 2)
	or a, a
	jr z, .Lbackground_scale_regular
	ld de, (_render_background_mul_value)
	ld (_render_background_mul_shift + 1), de
	ld hl, (_render_background_mul_shift)
	jr .Lbackground_scale_apply_sign

.Lbackground_scale_regular:
	ld hl, (_render_background_mul_factor)
	ld a, (_render_background_mul_value)
	call .Lscale_delta_8
	ld (_render_background_mul_result), hl

	ld bc, 0
	ld a, (_render_background_mul_value + 1)
	ld b, a
	ld a, (_render_background_mul_factor)
	ld c, a
	mlt bc
	...
	ld a, (_render_background_mul_value + 1)
	ld d, a
	ld a, (_render_background_mul_factor + 1)
	ld e, a
	mlt de
	ld (_render_background_mul_shift + 1), de
	ld hl, (_render_background_mul_shift)
	add hl, bc
	ld de, (_render_background_mul_result)
	add hl, de

.Lbackground_scale_apply_sign:
	ld a, (_render_background_mul_negative)
	or a, a
	ret z
	jp .Lbackground_negate_hl

/* Seed the X-world-line affine recurrence exactly like the former C setup. */
.Lbackground_init_x_lines:
	ld hl, (_render_background_state + BG_DIR_Y)
	ld de, 0
	or a, a
	sbc hl, de
	jr nz, .Lbackground_init_x_projected

	ld hl, (_render_background_state + BG_PLAYER_X)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_INPUT_NEAR), hl
	ld (_render_background_state + BG_X_INPUT_FAR), hl
	ld de, (_render_background_state + BG_INV_X)
	call .Lbackground_scale_mul
	ld (_render_background_state + BG_X_LINE_NEAR), hl
	ld (_render_background_state + BG_X_LINE_FAR), hl
	ld hl, (_render_background_state + BG_INV_X)
	ld (_render_background_state + BG_X_LINE_STEP), hl
	ld a, (_render_background_state + BG_DIR_X + 2)
	bit 7, a
	ret z
	ld hl, (_render_background_state + BG_X_LINE_NEAR)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_LINE_NEAR), hl
	ld (_render_background_state + BG_X_LINE_FAR), hl
	ld hl, (_render_background_state + BG_X_LINE_STEP)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_LINE_STEP), hl
	ret

.Lbackground_init_x_projected:
	ld hl, (_render_background_state + BG_PLAYER_X)
	ld de, (_render_background_state + BG_X_NEAR)
	add hl, de
	ld (_render_background_state + BG_X_INPUT_NEAR), hl
	ld de, (_render_background_state + BG_INV_Y)
	call .Lbackground_scale_mul
	ld (_render_background_state + BG_X_LINE_NEAR), hl

	ld hl, (_render_background_state + BG_PLAYER_X)
	ld de, (_render_background_state + BG_X_FAR)
	add hl, de
	ld (_render_background_state + BG_X_INPUT_FAR), hl
	ld de, (_render_background_state + BG_INV_Y)
	call .Lbackground_scale_mul
	...
	ld hl, (_render_background_state + BG_INV_Y)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_LINE_STEP), hl
	ld a, (_render_background_state + BG_DIR_Y + 2)
	bit 7, a
	ret z
	ld hl, (_render_background_state + BG_X_LINE_NEAR)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_LINE_NEAR), hl
	ld hl, (_render_background_state + BG_X_LINE_FAR)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_X_LINE_FAR), hl
	ld hl, (_render_background_state + BG_X_LINE_STEP)
	call .Lbackground_negate_hl
...
/* Seed the Y-world-line affine recurrence exactly like the former C setup. */
.Lbackground_init_y_lines:
	ld hl, (_render_background_state + BG_DIR_X)
	ld de, 0
	or a, a
	sbc hl, de
	jr nz, .Lbackground_init_y_projected

	ld hl, (_render_background_state + BG_PLAYER_Y)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_Y_INPUT_NEAR), hl
	ld (_render_background_state + BG_Y_INPUT_FAR), hl
	ld de, (_render_background_state + BG_INV_Y)
	call .Lbackground_scale_mul
	ld (_render_background_state + BG_Y_LINE_NEAR), hl
	ld (_render_background_state + BG_Y_LINE_FAR), hl
	ld hl, (_render_background_state + BG_INV_Y)
	ld (_render_background_state + BG_Y_LINE_STEP), hl
	ld a, (_render_background_state + BG_DIR_Y + 2)
	bit 7, a
	ret z
	ld hl, (_render_background_state + BG_Y_LINE_NEAR)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_Y_LINE_NEAR), hl
	ld (_render_background_state + BG_Y_LINE_FAR), hl
	ld hl, (_render_background_state + BG_Y_LINE_STEP)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_Y_LINE_STEP), hl
	ret

.Lbackground_init_y_projected:
	ld hl, (_render_background_state + BG_PLAYER_Y)
	call .Lbackground_negate_hl
	ld de, (_render_background_state + BG_Y_NEAR)
	or a, a
	sbc hl, de
	ld (_render_background_state + BG_Y_INPUT_NEAR), hl
	ld de, (_render_background_state + BG_INV_X)
	...
	ld hl, (_render_background_state + BG_PLAYER_Y)
	call .Lbackground_negate_hl
	ld de, (_render_background_state + BG_Y_FAR)
	or a, a
	sbc hl, de
	ld (_render_background_state + BG_Y_INPUT_FAR), hl
	ld de, (_render_background_state + BG_INV_X)
	...
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_Y_LINE_NEAR), hl
	ld hl, (_render_background_state + BG_Y_LINE_FAR)
	call .Lbackground_negate_hl
	ld (_render_background_state + BG_Y_LINE_FAR), hl
	ld hl, (_render_background_state + BG_Y_LINE_STEP)
	call .Lbackground_negate_hl
...
/* Generic X-line loop: subtract one source cell after each drawn line. */
.Lbackground_draw_x_projected:
	ld a, BACKGROUND_GRID_LINES
	ld (_render_background_state + BG_LINE_COUNT), a
.Lbackground_x_projected_loop:
	ld hl, (_render_background_state + BG_X_LINE_NEAR)
	ld de, (_render_background_state + BG_X_LINE_FAR)
	call _render_asm_add_projected_grid_segment_registers
	ld a, (_render_background_state + BG_LINE_COUNT)
	dec a
	ld (_render_background_state + BG_LINE_COUNT), a
	ret z

	ld de, (_render_background_state + BG_X_LINE_STEP)
	ld hl, (_render_background_state + BG_X_LINE_NEAR)
	add hl, de
	ld (_render_background_state + BG_X_LINE_NEAR), hl
	ld hl, (_render_background_state + BG_X_LINE_FAR)
	...
	ld de, -256
	ld hl, (_render_background_state + BG_X_INPUT_NEAR)
	add hl, de
	ld (_render_background_state + BG_X_INPUT_NEAR), hl
	ld hl, (_render_background_state + BG_X_INPUT_FAR)
	add hl, de
	ld (_render_background_state + BG_X_INPUT_FAR), hl

	ld hl, _render_background_state + BG_X_INPUT_NEAR
	ld de, _render_background_state + BG_X_LINE_NEAR
	call .Lbackground_repair_down_crossing
	ld hl, _render_background_state + BG_X_INPUT_FAR
	ld de, _render_background_state + BG_X_LINE_FAR
	call .Lbackground_repair_down_crossing
	jr .Lbackground_x_projected_loop

/*
 * HL=input address, DE=line address. Recompute only for -255..0, the one
 * interval where signed truncation breaks the otherwise exact recurrence.
 */
.Lbackground_repair_down_crossing:
	ld (_render_background_input_pointer), hl
	ld (_render_background_line_pointer), de
	ld a, (hl)
	ld (_render_background_cross_low), a
	inc hl
	ld a, (hl)
	cp a, 0xff
	jr nz, .Lbackground_repair_down_zero
	inc hl
	ld a, (hl)
	cp a, 0xff
	ret nz
	ld a, (_render_background_cross_low)
	or a, a
	ret z
	jr .Lbackground_repair_x_value

.Lbackground_repair_down_zero:
	or a, a
	ret nz
	inc hl
	ld a, (hl)
...
.Lbackground_repair_x_value:
	ld hl, (_render_background_input_pointer)
	ld hl, (hl)
	ld de, (_render_background_state + BG_INV_Y)
	call .Lbackground_scale_mul
	ld a, (_render_background_state + BG_DIR_Y + 2)
	bit 7, a
	jr z, .Lbackground_repair_x_store
	call .Lbackground_negate_hl
.Lbackground_repair_x_store:
	ld de, (_render_background_line_pointer)
	ex de, hl
	ld (hl), de
	ret

/* Generic Y-line loop: add one source cell after each drawn line. */
.Lbackground_draw_y_projected:
	ld a, BACKGROUND_GRID_LINES
	ld (_render_background_state + BG_LINE_COUNT), a
.Lbackground_y_projected_loop:
	ld hl, (_render_background_state + BG_Y_LINE_NEAR)
	ld de, (_render_background_state + BG_Y_LINE_FAR)
	call _render_asm_add_projected_grid_segment_registers
	ld a, (_render_background_state + BG_LINE_COUNT)
	dec a
	ld (_render_background_state + BG_LINE_COUNT), a
	...
	ld de, (_render_background_state + BG_Y_LINE_STEP)
	ld hl, (_render_background_state + BG_Y_LINE_NEAR)
	add hl, de
	ld (_render_background_state + BG_Y_LINE_NEAR), hl
	ld hl, (_render_background_state + BG_Y_LINE_FAR)
	...
	ld de, 256
	ld hl, (_render_background_state + BG_Y_INPUT_NEAR)
	add hl, de
	ld (_render_background_state + BG_Y_INPUT_NEAR), hl
	ld hl, (_render_background_state + BG_Y_INPUT_FAR)
	add hl, de
	ld (_render_background_state + BG_Y_INPUT_FAR), hl

	ld hl, _render_background_state + BG_Y_INPUT_NEAR
	ld de, _render_background_state + BG_Y_LINE_NEAR
	call .Lbackground_repair_up_crossing
	ld hl, _render_background_state + BG_Y_INPUT_FAR
	ld de, _render_background_state + BG_Y_LINE_FAR
	call .Lbackground_repair_up_crossing
	jr .Lbackground_y_projected_loop

/* HL=input address, DE=line address. Recompute only for 0..255. */
.Lbackground_repair_up_crossing:
	ld (_render_background_input_pointer), hl
	ld (_render_background_line_pointer), de
	inc hl
	ld a, (hl)
	...
	ld a, (_render_background_state + BG_DIR_X + 2)
	bit 7, a
	jr z, .Lbackground_repair_y_store
	call .Lbackground_negate_hl
.Lbackground_repair_y_store:
	ld de, (_render_background_line_pointer)
	ex de, hl
	ld (hl), de
	ret

/* Exact-cardinal loops need only one affine distance and no zero repair. */
.Lbackground_draw_x_axis:
	ld a, BACKGROUND_GRID_LINES
	ld (_render_background_state + BG_LINE_COUNT), a
.Lbackground_x_axis_loop:
	ld hl, (_render_background_state + BG_X_LINE_NEAR)
	call .Lbackground_draw_cardinal_line
	ld a, (_render_background_state + BG_LINE_COUNT)
	dec a
	ld (_render_background_state + BG_LINE_COUNT), a
	ret z
	...
	add hl, de
	ld (_render_background_state + BG_X_LINE_NEAR), hl
	jr .Lbackground_x_axis_loop

.Lbackground_draw_y_axis:
	ld a, BACKGROUND_GRID_LINES
	ld (_render_background_state + BG_LINE_COUNT), a
.Lbackground_y_axis_loop:
	ld hl, (_render_background_state + BG_Y_LINE_NEAR)
	call .Lbackground_draw_cardinal_line
	ld a, (_render_background_state + BG_LINE_COUNT)
	dec a
	...
	add hl, de
	ld (_render_background_state + BG_Y_LINE_NEAR), hl
	jr .Lbackground_y_axis_loop

/* HL=positive distance candidate; tail-call the unchanged packed row writer. */
.Lbackground_draw_cardinal_line:
	ld (_render_background_mul_value), hl
	ld a, (_render_background_mul_value + 2)
	bit 7, a
	ret nz
	...
	ret c
	ld hl, (_render_background_mul_value)
	ld de, BACKGROUND_FAR_DISTANCE + 1
	or a, a
	sbc hl, de
	ret nc
	ld hl, (_render_background_mul_value)
	ld de, _render_wall_scale_profile_index
	add hl, de
	ld a, (hl)
	ld hl, 0
	ld l, a
	...
	rr e
	ld a, e
	add a, BACKGROUND_SCREEN_CENTER_Y
	jp _render_asm_draw_horizontal_grid_pair_register

	.size _render_asm_draw_background_grid, .-_render_asm_draw_background_grid
...
_render_asm_repair_horizon:
	...
	.global _render_asm_draw_horizontal_grid_pair_register
	.type _render_asm_draw_horizontal_grid_pair, @function
	.type _render_asm_draw_horizontal_grid_pair_register, @function
...
_render_asm_draw_horizontal_grid_pair:
	call __frameset0
	ld a, (ix + 6)
	call _render_asm_draw_horizontal_grid_pair_register
	pop ix
	ret

/* Internal hot ABI: A=screen_y. */
_render_asm_draw_horizontal_grid_pair_register:
	...
	.size _render_asm_draw_horizontal_grid_pair_register, .-_render_asm_draw_horizontal_grid_pair_register
...
_render_asm_axis_boundary:
	.zero 1
	.local _render_grid_input_near
_render_grid_input_near:
	.zero 3
	.local _render_grid_input_far
_render_grid_input_far:
	.zero 3
...
_render_background_restore_interrupts:
	.zero 1
	.local _render_background_state
_render_background_state:
	.zero BG_STATE_SIZE
	.local _render_background_mul_negative
_render_background_mul_negative:
	.zero 1
	.local _render_background_mul_value
_render_background_mul_value:
	.zero 3
	.local _render_background_mul_factor
_render_background_mul_factor:
	.zero 3
	.local _render_background_mul_result
_render_background_mul_result:
	.zero 3
	.local _render_background_mul_shift
_render_background_mul_shift:
	.zero 4
	.local _render_background_input_pointer
_render_background_input_pointer:
	.zero 3
	.local _render_background_line_pointer
_render_background_line_pointer:
	.zero 3
	.local _render_background_cross_low
_render_background_cross_low:
	.zero 1
...
_render_portal_bottom_start:
	...
	.extern _render_wall_scale_profile_index
	.extern _render_wall_scale_profiles

...
.Lgrid_clipped_draw:
	/*
	 * Draw both already-clipped lines locally.  Almost every reachable pair
	 * is horizontal-major, so normalize X and traverse those two independent
	 * Bresenham states together.  The paired loop retains separate errors,
	 * Y deltas, and row strides; it only shares the X pointer advance and loop
	 * control.  Rare vertical/mixed-major states fall back to the exact
	 * single-line helper below.
	 */
	call .Lgrid_draw_dynamic_pair_noclip
...
/*
 * Draw an already-clipped floor/ceiling pair with shared X traversal.
 * Inputs live in the _render_grid_* scratch slots populated by the clipper.
 * Both lines are still independent GraphX Bresenham rasters; only the common
 * left-to-right pixel count and pointer increments are coalesced.
 */
.Lgrid_draw_dynamic_pair_noclip:
	/* Normalize the shared endpoints exactly as gfx_Line_NoClip does. */
	ld hl, (_render_grid_far_x)
	ld de, (_render_grid_near_x)
	or a, a
	sbc hl, de
	jr c, .Lgrid_dynamic_pair_far_left

	/* Endpoint one is leftmost (equal X is swapped by GraphX too). */
	...
	jr .Lgrid_dynamic_pair_y_setup

.Lgrid_dynamic_pair_far_left:
	/* Endpoint zero is leftmost. HL already holds x0-x1 as a wrapped value. */
	ld hl, (_render_grid_near_x)
...
.Lgrid_dynamic_pair_y_setup:
	/* Floor: signed delta, absolute delta, negative delta, and row stride. */
	ld a, (_render_grid_clip_floor_result)
	ld b, a
	ld a, (_render_grid_clip_ceiling_result)
	sub a, b
	ld de, 320
	jr nc, .Lgrid_dynamic_pair_floor_positive
	neg
	ld de, -320
.Lgrid_dynamic_pair_floor_positive:
	ld (_render_grid_clip_floor_delta), a
	ld (.Lgrid_dynamic_pair_floor_row_step + 1), de
	ld hl, 0
	ld l, a
	ex de, hl
	or a, a
	...
	/* Ceiling: retain its independently rounded signed delta. */
	ld a, (_render_grid_clip_quotient)
	ld b, a
	ld a, (_render_grid_clip_ceiling_magnitude)
	sub a, b
	ld de, 320
	jr nc, .Lgrid_dynamic_pair_ceiling_positive
	neg
	ld de, -320
.Lgrid_dynamic_pair_ceiling_positive:
	ld (_render_grid_clip_ceiling_delta), a
	ld (.Lgrid_dynamic_pair_ceiling_row_step + 1), de
	ld hl, 0
	ld l, a
	ex de, hl
	or a, a
	...
	/* The paired fast path is exact only when both lines are horizontal-major. */
	ld hl, 0
	ld a, (_render_grid_clip_floor_delta)
	ld l, a
	ld de, (_render_grid_clip_d)
	or a, a
	sbc hl, de
	jp nc, .Lgrid_dynamic_pair_fallback
	ld hl, 0
	...
	/* Preserve the existing packed-run win whenever either line qualifies. */
	ld a, (_render_grid_clip_d + 1)
	bit 0, a
	jr z, .Lgrid_dynamic_pair_pointer_setup
	...
	ld a, (_render_grid_clip_floor_delta)
	or a, a
	jr z, .Lgrid_dynamic_pair_check_ceiling_run
	cp a, 20
	jp nc, .Lgrid_dynamic_pair_check_ceiling_run
	ld a, b
	ld c, a
	ld a, (_render_grid_clip_floor_delta)
	cp a, c
	jp c, .Lgrid_dynamic_pair_fallback
	jp z, .Lgrid_dynamic_pair_fallback
.Lgrid_dynamic_pair_check_ceiling_run:
	ld a, (_render_grid_clip_ceiling_delta)
	or a, a
	jr z, .Lgrid_dynamic_pair_pointer_setup
	cp a, 20
	jr nc, .Lgrid_dynamic_pair_pointer_setup
	ld a, b
	ld c, a
	ld a, (_render_grid_clip_ceiling_delta)
	cp a, c
	...
	/* Resolve the two leftmost pixel pointers through the existing row LUT. */
.Lgrid_dynamic_pair_pointer_setup:
	ld a, (_render_grid_clip_floor_result)
	call .Lgrid_dynamic_pair_row_pointer
	ld (_render_grid_floor_row), hl
	ld a, (_render_grid_clip_quotient)
	call .Lgrid_dynamic_pair_row_pointer
	ld (_render_grid_ceiling_row), hl

	/* IX and IY hold the two independent floor(dx/2) errors. */
	...
	/* Keep the ceiling pointer and its negative delta in the alternate set. */
	ld hl, (_render_grid_ceiling_row)
	ld de, (_render_grid_clip_remainder)
	exx
...
.Lgrid_dynamic_pair_horizontal_loop:
	ld (hl), GRID_FLOOR_COLOR
	inc hl
	add ix, de
	jr c, .Lgrid_dynamic_pair_floor_ready
.Lgrid_dynamic_pair_floor_row_step:
	ld de, 0
	add hl, de
	ld de, (_render_grid_clip_d)
	add ix, de
	ld de, (_render_grid_clip_n)
.Lgrid_dynamic_pair_floor_ready:

	exx
	ld (hl), GRID_CEILING_COLOR
	inc hl
	add iy, de
	jr c, .Lgrid_dynamic_pair_ceiling_ready
.Lgrid_dynamic_pair_ceiling_row_step:
	ld de, 0
	add hl, de
	ld de, (_render_grid_clip_d)
	add iy, de
	ld de, (_render_grid_clip_remainder)
.Lgrid_dynamic_pair_ceiling_ready:
	exx
	dec bc
	ld a, b
	or a, c
	ret z
	jr .Lgrid_dynamic_pair_horizontal_loop

/* A=row, return HL=draw_buffer + row*320 + shared left X. */
.Lgrid_dynamic_pair_row_pointer:
	ld hl, 0
	ld l, a
	add hl, hl
	add hl, hl

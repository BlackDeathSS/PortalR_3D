...
static LevelView level;
static LevelSource level_source;

_Static_assert(
    sizeof(EngineState) + sizeof(LevelView) + 4096u < 150u * 1024u,
    "Static state plus reserved CEdev stack exceeds 150 KiB"
);
...
int main(void) {
    ...
    if (!level_source_open(&level, &level_source) || !engine_init(&engine, &level)) {
        level_source_close(&level_source);
        return 1;
    }
    ...
    engine_render(&engine, &level, displayed_fps, level_source.external);
    gfx_SwapDraw();
    ...
    while ((kb_Data[6] & kb_Clear) == 0) {
        ...
        vertical_axis = (int8_t)(
            ((kb_Data[7] & kb_Up) != 0) - ((kb_Data[7] & kb_Down) != 0)
        );
        move_axis = vertical_axis;
        look_axis = (int8_t)(

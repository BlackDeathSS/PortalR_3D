...
def main() -> None:
    ...
    interrupt_bytes = values["BACKGROUND_INTERRUPT_BYTES"]
    ...
    if ceiling_pushes * 3 != ceiling_bytes - interrupt_bytes:
        raise AssertionError("ceiling PUSH run does not stop at interrupt reserve")
    if interrupt_bytes < 4000:
        raise AssertionError("interrupt stack reserve fell below GraphX's minimum")
    ...
    if sp != interrupt_bytes:
        raise AssertionError(
            f"fast ceiling stopped at byte {sp}, expected {interrupt_bytes}"
        )

    # LDDR uses the first fast-filled ceiling byte as a one-byte overlapping
    # source and propagates it backward through the reserved prefix.
    source_index = sp
    destination_index = sp - 1
    for _ in range(interrupt_bytes):
        buffer[destination_index] = buffer[source_index]
        source_index -= 1
        destination_index -= 1
    ...
    print(
        ...
        f"{writes} total writes, {interrupt_bytes}-byte interrupt reserve"
    )

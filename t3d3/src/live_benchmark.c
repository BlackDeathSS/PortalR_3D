...
#endif
#ifndef T3D3_BODY_BENCHMARK
#define T3D3_BODY_BENCHMARK 0
#endif
...
#define LIVE_RESULT_NAME "T3D3LIV"
#define LIVE_TEMP_NAME "T3D3TMP"
...
static uint8_t live_warmup(void) {
    ...
    for (frame = 0; frame < LIVE_WARMUP_FRAMES; ++frame) {
        ...
        (void)section;
        (void)engine_update(
            &live_state, move_axis, turn_axis, look_axis, buttons, 0u,
...
static void live_write_header(
    ...
    header[44] = T3D3_RENDER_WIDTH;
    header[45] = T3D3_RENDER_HEIGHT;
    header[46] = T3D3_RENDER_SCALE;
    header[47] = 1;
...
static void live_show_failure(const char *message) {
    os_ClrHome();
    os_SetCursorPos(0, 0);
    os_PutStrFull("T3D3 benchmark failed");
    os_SetCursorPos(2, 0);
...
static void live_show_result(
    ...
    os_ClrHome();
    os_SetCursorPos(0, 0);
    os_PutStrFull(saved ? "T3D3 live benchmark done" :
        "T3D3 benchmark save failed");
    os_SetCursorPos(2, 0);
...
int true3d_live_benchmark_run(void) {
    ...
    if (!live_prepare_report()) {
        live_show_failure("No room for T3D3LIV.");
        return 1;
    }
    live_write_sections();
...
#if TRUE3D_LIVE_AUTOTEST_HOLD
    /* Preserve the report in RAM even when a diagnostic gate fails so the
       host decoder can identify the exact divergent frame/state. */
    (void)diagnostics_ok;
    saved = live_commit_report();
#else

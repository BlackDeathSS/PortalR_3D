...
static __attribute__((noinline)) uint8_t prepare_leaf_face_polygon(
    const RenderLayer *layer,
    uint8_t face_index,
    uint8_t face_offset,
    DrawPolygon *polygon
...
static __attribute__((noinline)) void collect_room_polygons(
    RenderLayer *layer,
    const Camera *camera,
...
/*
 * A depth-one view cannot contain another aperture.  Scan its convex faces
 * directly into the destination instead of first writing 192 bytes of span
 * state per face and then walking those rows a second time in fill_polygon.
 */
static __attribute__((noinline)) void rasterize_fill_leaf_full(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
...
static __attribute__((noinline)) void rasterize_fill_leaf_lod(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t shift = layer->lod_shift;
    ...
    for (row = first_row; row <= last_row; row += step) {
        ...
        if (first_column <= last_column && last_column >= sample_origin) {
            uint8_t target_first = first_column <= sample_origin ? 0 :
                (uint8_t)((first_column - sample_origin + step - 1u) >> shift);
            uint8_t target_last =
            ...
            if (target_first <= target_last) {
                memset(
                    &portal_lod_frame[
                        (uint16_t)(row >> shift) * PORTAL_LOD_STRIDE +
                        target_first
                    ],
...
static __attribute__((noinline)) void render_portal_leaf_fused(
    const Camera *camera,
    RenderLayer *layer,
    uint8_t skipped_face
) {
    ...
    for (offset = 0; offset < ROOM_FACE_COUNT; ++offset) {
        ...
        if (face_index == skipped_face ||
            !prepare_leaf_face_polygon(layer, face_index, offset, polygon)) {
            continue;
        }
        if (layer->lod_shift == 0) {
            rasterize_fill_leaf_full(polygon, layer);
        } else {
            rasterize_fill_leaf_lod(polygon, layer);
        }
    }
    if (layer->lod_shift != 0) {
...
static __attribute__((noinline)) void render_camera(
    const Camera *camera,
    uint8_t depth,
    uint8_t skipped_face
) {
    ...
    if (depth < PORTAL_RECURSION_LIMIT) {
        for (index = layer->solid_count; index < layer->count; ++index) {
            ...
            render_portal_leaf_fused(&destination, child, destination_face);
            RENDER_BENCHMARK_SWITCH(TRUE3D_BENCH_PORTAL_FILL);

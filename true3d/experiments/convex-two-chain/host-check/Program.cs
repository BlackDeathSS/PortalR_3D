...
bool BeginChain(Pt[] points, Chain chain, int row, int step)
{
    while (chain.EdgesLeft != 0)
    {
        ...
        int xStep = EdgeStep(b.X - a.X, b.Y - a.Y);
        chain.Last = last;
        chain.X = unchecked(a.X + ((xStep * (row * Fixed + Fixed / 2 - a.Y)) >> 8));
        chain.Advance = unchecked(xStep * step);
        return true;
    }
    return false;
}
...
Result Candidate(Pt[] points, Layer layer)
{
    ...
    for (int row = bounds.First; row <= bounds.Last; row += bounds.Step)
    {
        ...
        if (row + bounds.Step <= first.Last) first.X = unchecked(first.X + first.Advance);
        if (row + bounds.Step <= second.Last) second.X = unchecked(second.X + second.Advance);
    }
    return FinishResult(bounds, layer, left, right);
}

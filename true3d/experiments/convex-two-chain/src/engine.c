...
static uint8_t raster_chain_begin_edge(
    ...
    while (chain->edges_left != 0) {
        ...
        /* A convex polygon has two monotone chains from its top vertex. */
        if (a.y > b.y) return 0;
...
static uint8_t rasterize_polygon(
    ...
    if (!raster_chain_begin_edge(
            &first_chain, polygon, first_row, step
        ) ||
        !raster_chain_begin_edge(
            &second_chain, polygon, first_row, step
        )) {
        return 0;
    }
    for (row = first_row; row <= last_row; row += step) {
        ...
        first_column = ceil_q8(
            clamp_projected(left_value) - FIXED_ONE / 2
        );
        last_column = floor_q8(
            clamp_projected(right_value) - FIXED_ONE / 2
        );

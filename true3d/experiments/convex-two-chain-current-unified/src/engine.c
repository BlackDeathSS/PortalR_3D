...
static uint8_t rasterize_polygon_lod(
    ...
    first_chain.vertex = top_vertex;
    first_chain.direction = 1;
    first_chain.edges_left = polygon->count;
    second_chain.vertex = top_vertex;
...
static uint8_t __attribute__((unused)) rasterize_polygon_chains_lod(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t step = (uint8_t)(1u << layer->lod_shift);
    ...
    for (index = 1; index < polygon->count; ++index) {
        ...
        if (polygon->point[index].y > maximum_y) {
            maximum_y = polygon->point[index].y;
        }
    }
    first_row = ceil_q8(minimum_y - FIXED_ONE / 2);
    last_row = floor_q8(maximum_y - 1 - FIXED_ONE / 2);
...
static uint8_t __attribute__((unused)) rasterize_polygon_chains_full(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t index;
    ...
    if (first_row < 0) first_row = 0;
    if (last_row >= active_render_height) last_row = active_render_height - 1;
    if (last_row < 0 || first_row > last_row) return 0;
...
static uint8_t rasterize_polygon_chains_unified(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t step = (uint8_t)(1u << layer->lod_shift);
...
static uint8_t rasterize_polygon(
    ...
    return rasterize_polygon_chains_unified(polygon, layer);
}

...
static uint8_t validation_force_root_clear;
static uint8_t validation_reference[RENDER_WIDTH * RENDER_HEIGHT];
#endif
LowFrame low_frame;
...
static void clear_render_layer(const RenderLayer *layer, uint8_t depth) {
    if (depth == 0) {
#if TRUE3D_CLEAR_VALIDATION
        if (!validation_force_root_clear) return;
#endif
        memset(
            low_frame.data,
            COLOR_VOID,
...
#if TRUE3D_CLEAR_VALIDATION
static uint32_t validation_random(uint32_t *state) {
...
static fixed_t validation_coordinate(
    uint32_t *random_state,
    fixed_t minimum,
    fixed_t maximum
) {
    uint24_t span = (uint24_t)(maximum - minimum - 2);

    return (fixed_t)(minimum + 1 + validation_random(random_state) % span);
}

static void validation_render_logical(const EngineState *state) {
    Camera camera;
    uint8_t row;
    ...
    camera.room = state->room;
    render_layers[0].first_row = 0;
    render_layers[0].last_row = RENDER_HEIGHT - 1;
    render_layers[0].bound_left = 0;
    render_layers[0].bound_right = RENDER_WIDTH - 1;
    render_layers[0].lod_shift = 0;
    render_layers[0].pixel_area = RENDER_WIDTH * RENDER_HEIGHT;
    for (row = 0; row < RENDER_HEIGHT; ++row) {
        render_layers[0].row_left[row] = 0;
        render_layers[0].row_right[row] = RENDER_WIDTH - 1;
    }
    render_camera(&camera, 0, NO_PORTAL);
}
...
uint24_t engine_validate_root_clear(
    ...
    *first_failed_sample = 0xFFFFu;
    *first_failed_pixel = 0xFFFFu;
    for (sample = 0; sample < sample_count; ++sample) {
        const Room *room;
        uint8_t initial_lod_0 = (uint8_t)(sample % 3u);
        uint8_t initial_lod_1 = (uint8_t)((sample / 3u) % 3u);
        uint16_t pixel;

        state->room = (uint8_t)(sample % room_count);
        room = &rooms[state->room];
        state->position.x = validation_coordinate(
            &random_state, room->minimum_x, room->maximum_x
        );
        state->position.y = validation_coordinate(
            &random_state, room->minimum_y, room->maximum_y
        );
        state->position.z = validation_coordinate(
            &random_state, room->minimum_z, room->maximum_z
        );
        /* 73 is odd, so every block of 256 samples covers every yaw. */
        state->yaw = (uint8_t)(sample * 73u);
        /* 37 is coprime to 129, covering every legal pitch repeatedly. */
        state->pitch = (int8_t)((sample * 37u) % 129u - 64);
        state->render_shift = 0;
        rebuild_camera_basis(state);

        portal_lod_state[0] = initial_lod_0;
        portal_lod_state[1] = initial_lod_1;
        validation_force_root_clear = 1;
        validation_render_logical(state);
        memcpy(validation_reference, low_frame.data, sizeof(validation_reference));

        for (pixel = 0; pixel < sizeof(validation_reference); ++pixel) {
            low_frame.data[pixel] = validation_reference[pixel] ^ 0x5Au;
        }
        portal_lod_state[0] = initial_lod_0;
        portal_lod_state[1] = initial_lod_1;
        validation_force_root_clear = 0;
        validation_render_logical(state);
        for (pixel = 0; pixel < sizeof(validation_reference); ++pixel) {
            if (low_frame.data[pixel] != validation_reference[pixel]) {
                if (failures == 0) {
                    *first_failed_sample = sample;
                    *first_failed_pixel = pixel;
                }
                ++failures;
            }
        }

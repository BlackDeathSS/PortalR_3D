...
static uint8_t clip_and_project(
    ...
    const ScreenPoint *source_screen,
    ...
    for (index = 0; index < source_count; ++index) {
        ...
        const CameraPoint *previous = &source[
            index == 0 ? source_count - 1 : index - 1
        ];
        ...
        if (current_inside != previous_inside) {
            ...
            if (output_count < MAX_POLYGON_VERTICES) {
                polygon->point[output_count++] =
                    project_camera_point(&intersection);
            }
        }
        if (current_inside && output_count < MAX_POLYGON_VERTICES) {
            polygon->point[output_count++] = source_screen[index];
        }
    }
...
static uint8_t append_face_polygon(
    ...
    for (index = 0; index < 4; ++index) {
        ...
        if (vertex_projectable[vertex]) {
            source_screen[index] = screen_vertices[vertex];
            polygon->point[index] = source_screen[index];
            ++inside_count;
        }
    }
    ...
    if (inside_count == 4) {
        polygon->count = 4;
    } else if (!clip_and_project(point, source_screen, 4, polygon)) {
        return 0;
    }
...
static void append_portal_polygon(
    ...
    polygon = &layer->polygon[layer->count];
    for (index = 0; index < 4; ++index) {
        if (point[index].depth >= NEAR_PLANE) {
            source_screen[index] = project_camera_point(&point[index]);
            polygon->point[index] = source_screen[index];
            ++inside_count;
        }
    }

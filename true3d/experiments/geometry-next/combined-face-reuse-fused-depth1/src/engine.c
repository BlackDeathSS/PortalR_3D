    ...
    for (index = 0; index < 4; ++index) {
        ...
        if (current_inside) {
            polygon->point[output_count++] =
                screen_vertices[face->vertex[index]];
        }
    }
    if (output_count < 3) return 0;
    polygon->count = output_count;
...
static uint8_t prepare_face_polygon(
    ...
    if (inside_count == 4) {
        polygon->count = 4;
    } else if (!clip_face_and_project(point, face, polygon)) {
        return 0;
    }
...
static void append_portal_polygon(
    ...
    for (index = 0; index < 4; ++index) {
        if (point[index].depth >= NEAR_PLANE) {
            polygon->point[index] = project_camera_point(&point[index]);
            ++inside_count;
        }
    }
...
static void rasterize_fill_polygon_lod(
    ...
    uint8_t step = (uint8_t)(1u << layer->lod_shift);
    uint8_t sample_origin = step >> 1;
    uint8_t maximum_column = (active_render_width >> layer->lod_shift) - 1u;
    int16_t first_row = polygon->sample_first_row;
    int16_t last_row = polygon->sample_last_row;
    int16_t row;
    RasterChain first_chain;
        ...
        if (last_column >= sample_origin) {
            uint8_t target_first = first_column <= sample_origin ? 0 :
                (uint8_t)((first_column - sample_origin + step - 1u) >>
                    layer->lod_shift);
            uint8_t target_last =
                (uint8_t)((last_column - sample_origin) >> layer->lod_shift);

            if (target_last > maximum_column) target_last = maximum_column;
            if (target_first <= target_last) {
                uint8_t target_row = (uint8_t)row >> layer->lod_shift;
                memset(
                    &portal_lod_frame[

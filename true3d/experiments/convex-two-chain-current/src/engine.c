...
static uint8_t rasterize_polygon_chains_lod(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t step = (uint8_t)(1u << layer->lod_shift);
...
static uint8_t rasterize_polygon_chains_full(
    DrawPolygon *polygon,
    const RenderLayer *layer
) {
    uint8_t index;
...
static uint8_t rasterize_polygon(
    ...
    if (layer->lod_shift == 0) {
        return rasterize_polygon_chains_full(polygon, layer);
    }
    return rasterize_polygon_chains_lod(polygon, layer);
}

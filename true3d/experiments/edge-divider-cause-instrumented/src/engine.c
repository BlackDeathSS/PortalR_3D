...
#if TRUE3D_RENDER_BENCHMARK
        /* Diagnostic-only packed counter.  Each field is five bits, which is
         * sufficient for the observed 3--11 fallbacks per rendered frame:
         *   bits 0..4   delta_y < 256
         *   bits 5..9   reciprocal-table range exceeded
         *   bits 10..14 wide delta_x (after the first two tests)
         */
        if (delta_y < FIXED_ONE) {
            RENDER_BENCHMARK_COUNT(edge_division_fallbacks, 1u);
        } else if (index >= EDGE_RECIPROCAL_SIZE) {
            RENDER_BENCHMARK_COUNT(edge_division_fallbacks, 32u);
        } else {
            RENDER_BENCHMARK_COUNT(edge_division_fallbacks, 1024u);
        }
#endif

...
void fill_root_color_rows_exact(
    const void *polygon,
    uint8_t first_row,
    uint8_t row_count,
    uint8_t color
);
void fill_clipped_color_rows_exact(
    const void *polygon,
    const void *layer,
    uint8_t first_row,
    uint8_t row_count,
...
void fill_root_one_aperture_rows_exact(
    const void *polygon,
    const void *aperture,
    uint8_t first_row,
    uint8_t row_count,
...
void fill_clipped_one_aperture_rows_exact(
    const void *polygon,
    const void *layer,
    const void *aperture,
    uint8_t first_row,
...
_Static_assert(offsetof(DrawPolygon, span_left) == 48u, "ASM span-left offset");
_Static_assert(offsetof(DrawPolygon, span_right) == 96u, "ASM span-right offset");
...
_Static_assert(offsetof(RenderLayer, row_left) == 1264u, "ASM clip-left offset");
_Static_assert(offsetof(RenderLayer, row_right) == 1312u, "ASM clip-right offset");
...
static void fill_polygon_color_rows(
    ...
    if (aperture_count == 0 && layer == &render_layers[0]) {
        fill_root_color_rows_exact(
            polygon,
            (uint8_t)first_row,
            (uint8_t)(last_row - first_row + 1),
            row_color
        );
        return;
    }
    if (aperture_count == 1 && layer == &render_layers[0]) {
        fill_root_one_aperture_rows_exact(
            polygon,
            aperture_polygon[0],
            (uint8_t)first_row,
            (uint8_t)(last_row - first_row + 1),
    ...
    if (aperture_count == 1) {
        fill_clipped_one_aperture_rows_exact(
            polygon,
            layer,
            aperture_polygon[0],
            (uint8_t)first_row,
